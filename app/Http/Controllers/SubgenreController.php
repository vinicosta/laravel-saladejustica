...
class SubgenreController extends Controller
{
    ...
    public function search(Request $request, Subgenre $model){
        if($request->search == ''){
            $subgenres = $this->getAll($model);
        ...
        $subgenres = $model->select('subgenres.*', 'genres.name as genre_name')
            ->join('genres', 'genres.id', '=', 'subgenres.genre_id')
            ->where('subgenres.name', 'LIKE', '%' . str_replace(' ', '%', trim($request->search)) . '%')
            ->orWhere('genres.name', 'LIKE', '%' . str_replace(' ', '%', trim($request->search)) . '%')
