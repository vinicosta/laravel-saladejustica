@extends('layouts.app', ['activePage' => 'books-management', 'titlePage' => __('Livros'), 'showSearch' => false, 'model' => 'issue'])
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header card-header-primary">
                        <h4 class="card-title ">
                            {{ $issue->name }} {{ $issue->subtitle != '' ? ' - ' . $issue->subtitle : '' }}
                        </h4>
                        <p class="card-category">
                            {{ listAuthors($authors) }}
                             - {{ $issue->publisher_name }}
                            @if($issue->previous_issue or $issue->next_issue)
                             - {{ $issue->title_name }}, volume {{ $issue->issue_number }}
                            @endif
                        </p>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-lg-6 col-sm-12 text-lg-left text-sm-center">
                                {{-- Edit issue --}}
                                <a href="{{ URL::to('issue/books/' . $issue->id . '/edit') }}" class="btn btn-sm btn-primary" title="Editar">
                                    <i class="material-icons" style="color: white">edit</i>
                                </a>

                                {{-- Delete issue --}}
                                <a href="{{ URL::to('issue/books/' . $issue->id . '/delete') }}" class="btn btn-sm btn-danger" title="Excluir">
                                ...
                                <a href="#" class="btn btn-sm btn-primary" style="display:{{ !$issue->readed ? '' : 'none' }}" id="btn-readed-{{ $issue->id }}" onclick="readed({{ $issue->id }}, {{ $issue->title_id }})" title data-original-title="Marcar como lido">
                                    <i class="material-icons" style="color: white">visibility</i>
                                </a>

                                {{-- Uncheck issue as readed --}}
                                <a href="#" class="btn btn-sm btn-primary" style="display:{{ $issue->readed ? '' : 'none' }}" id="btn-unreaded-{{ $issue->id }}" onclick="unreaded({{ $issue->id }})" title="Marcar como não lido" data-original-title="Marcar como não lido">
                            ...
                            <div class="col-lg-6 col-sm-12 text-lg-right text-sm-center">
                                ...
                                <a href="{{ URL::to('issue/books/' . $issue->first_issue) }}" class="btn btn-sm btn-warning">
                                ...
                                <a href="{{ URL::to('issue/books/' . $issue->previous_issue) }}" class="btn btn-sm btn-warning">
                                    <i class="material-icons" style="color: white" title="Anterior">chevron_left</i>
                                </a>
                                @endif

                                {{-- Show title --}}
                                <a href="{{ URL::to('title/books/' . $issue->title_id) }}" class="btn btn-sm btn-warning" title="Ver todos">
                                    <i class="material-icons" style="color: white">grid_on</i>
                                </a>

                                {{-- Show next issue --}}
                                @if($issue->next_issue)
                                <a href="{{ URL::to('issue/books/' . $issue->next_issue) }}" class="btn btn-sm btn-warning">
                                ...
                                {{-- Show last issue --}}
                                @if($issue->last_issue)
                                <a href="{{ URL::to('issue/books/' . $issue->last_issue) }}" class="btn btn-sm btn-warning">
