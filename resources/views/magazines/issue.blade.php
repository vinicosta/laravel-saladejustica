<div id="issue-{{ $issue->id }}">
            ...
            <div class="row">
                @switch($result)
                    @case('titles')
                        ...
                        <div class="col-6 text-center" style="display:{{ !$issue->reading ? 'block' : 'none' }}"
                            id="btn-reading-{{ $issue->id }}">
                            <button class="btn btn-primary btn-fab btn-fab-mini btn-link"
                                title="Adicionar à minha lista de leitura" onclick="reading({{ $issue->id }})">
                    ...
                    @case('issues')
                        ...
                        <div class="col-6 text-center" style="display:{{ !$issue->readed ? 'block' : 'none' }}"
                            ...
                            <button class="btn btn-primary btn-fab btn-fab-mini btn-link" title="Marcar como lido"
                                onclick="readed({{ $issue->id }}, {{ $issue->title_id }}, 'issue-{{ $issue->id }}', '{{ URL::to('title/magazines/' . $issue->title_id . '/next') }}')">
    ...
    <div class="text-center">
        <a
            href="{{ $result == 'titles' ? URL::to('title/magazines/' . $issue->id) : URL::to('issue/magazines/' . $issue->id) }}">
